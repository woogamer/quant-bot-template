...
> 이 전략은 **샘플**입니다. 그대로 써도 되고, 참고만 하고 자기만의 전략을 만들어도 됩니다.
> `/new-strategy` 명령으로 Claude에게 새 전략을 요청할 수 있습니다.

현재 `my_strategy.py`에는 **추세 적응형 데이트레이딩 전략**이 구현되어 있습니다.
MA10(10일 이동평균)의 기울기로 종목별 장세(상승/횡보/하락)를 판단하고,
장세에 맞는 방식으로 진입한 뒤 당일 안에 청산하는 전략입니다.
